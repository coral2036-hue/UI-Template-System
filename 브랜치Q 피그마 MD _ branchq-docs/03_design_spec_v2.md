...
│ [카카오톡] [MMS]   [    ]       │
```
→ [카카오톡] 클릭 시 M8 kakao-alimtalk 오픈
→ [MMS] 클릭 시 M7 mms 오픈
→ [이메일] 클릭 시 M5 email 오픈
...
### M5 email (수정안: Q&A 전체 렌더링 + PDF/Excel 자동 첨부)
```
│ 이메일 전송                  [✕] │
│ 받는 사람: [드롭다운+직접입력]    │
│ 제목: [질문 텍스트 자동 채움]     │
...
│                    [취소] [발송]  │
```
상세: `13_MMS_카카오_알림톡_업무흐름도.md`

### M8 kakao-alimtalk (카카오 알림톡)
```
│ 카카오 알림톡 전송          [✕] │
│ 받는 사람: [이름+전화번호 드롭]   │
│ ─ 알림톡 미리보기 ──────────── │
...
│                    [취소] [전송]  │
```
템플릿: 단일 고정 (선택 드롭다운 없음)
CTA 버튼 → AI 아나보 상세 화면 이동 (`/detail/{responseId}`)
상세: `13_MMS_카카오_알림톡_업무흐름도.md`
